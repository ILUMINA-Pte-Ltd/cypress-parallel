...
async function getTestSuitePaths() {
  let suitePathsArr = Array.isArray(settings.testSuitesPaths)
    ? settings.testSuitesPaths
    : settings.testSuitesPaths.split(',');
  let fileList = [];
  for (let suitePath of suitePathsArr) {
    let globPattern = false;
    if (suitePath.includes('*')) {
      globPattern = true;
    } else if (
      fs.existsSync(suitePath) &&
      fs.lstatSync(suitePath).isDirectory()
    ) {
      console.log(
        'DEPRECATED: using directory path is deprecated and will be removed, switch to glob pattern'
      );
      globPattern = true;
      suitePath = path.join(suitePath, '**', '*');
    }

    if (globPattern) {
      if (settings.isVerbose) {
        console.log(`Using pattern ${suitePath} to find test suites`);
      }
      const paths = await glob(suitePath, { ignore: 'node_modules/**' });
      fileList = fileList.concat(paths);
    } else {
      fileList.push(suitePath);
    }
  }
